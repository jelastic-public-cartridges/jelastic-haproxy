...
function _enableSSL(){
        ...
        sed -i '/bind :443 ssl crt/d' ${CARTRIDGE_HOME}/versions/$Version/conf/haproxy.conf;
        sed -i '/bind :443 ssl crt/d' ${CARTRIDGE_HOME}/versions/$Version/haproxy.conf.default;
        sed -i '/bind :80/a bind :443 ssl crt /var/lib/jelastic/SSL/jelastic.pem' ${CARTRIDGE_HOME}/versions/$Version/conf/haproxy.conf;
        sed -i '/bind :80/a bind :443 ssl crt /var/lib/jelastic/SSL/jelastic.pem' ${CARTRIDGE_HOME}/versions/$Version/haproxy.conf.default;
